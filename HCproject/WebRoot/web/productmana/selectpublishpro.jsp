  ...
  <head>
	...
	<link href="should_be_excluded/css/style.css" rel="stylesheet"
		...
		<script>
					...
					if(icount.trim()!=0){
					  document.getElementById("a1").href="web/productmana/selectpublishpro.jsp?page="+icount;
					  }else{
					   document.getElementById("a1").href="web/productmana/selectpublishpro.jsp?page=1";
					  }
					}
			...
			var x=document.getElementsByName("checkid");
			  ...
			  if(flag){
				...
				var f1=document.getElementById('proForm');
					f1.action="ProManaServlet?flag=shangxian";
					f1.submit();
				}
				}else{
				 alert('未选择产品');
			...
			var x=document.getElementsByName("checkid");
			  ...
			  if(flag){
				...
				alert('下线成功');
				var f1=document.getElementById('proForm');
					f1.action="ProManaServlet?flag=xiaxian";
					f1.submit();
				}
				}else{
				 alert('未选择产品');
			...
			function oneshangxian(checkedid){
			 var a2=document.getElementById('proForm');
			 if(confirm("您确定上线吗？")){
			    alert('上线成功！');
			 	a2.action="ProManaServlet?flag=shangxian&&checkid="+checkedid;
			 	a2.submit();
			 }
			}
			function onexiaxian(checkedid){
			 var a3=document.getElementById('proForm');
			 if(confirm("您确定下线吗？")){
			    alert('下线成功！');
			 	a3.action="ProManaServlet?flag=xiaxian&&checkid="+checkedid;
			 	a3.submit();
			 }
			}
			...
			<div class="dclass_container" id="div_rela_table">
				<div class="dclass_container_header">
					<div class="dclass_container_title">
						农产品上下线&nbsp;&nbsp;【<a href="web/productmana/publishpro.jsp">返回上一页列表</a>】
					</div>
				</div>
				<div class="dclass_container_content">

					<div class="actions">
						<input  type="button" class="btn1" value="一键上线"
							onclick="shangxian()" title="上线农产品..." />
						
						<input  type="button" class="btn1" value="一键下线"
							title="下线农产品..." onclick="xiaxian()"/>
					</div>
					...
					<table class="dclass_data">
						...
						<c:forEach var="pro" items="${prolist}">
							...
							${pro.pname}
								...
								<td>
								<c:if test="${pro.zhuangtai==1}">
								<a  href="javascript:void(0);" onclick="onexiaxian(${pro.id})">下线</a>
								</c:if>
								<c:if test="${pro.zhuangtai==2}">
								<a   href="javascript:void(0);" onclick="oneshangxian(${pro.id})">上线</a>
								</c:if>	
					...
					<div class="dclass_pager ">
						<span class="dclass_pager_nav">  <a href="web/productmana/selectpublishpro.jsp?page=1" >首页</a> </span>
						<%
							if ((view.getCurrentPage() - 1) <= 0) {
						 %>
						<span class="dclass_pager_nav"> <a href="web/productmana/selectpublishpro.jsp?page=1">上一页</a>  </span>
						<%}else{ %>
						 <span class="dclass_pager_nav"><a href="web/productmana/selectpublishpro.jsp?page=<%=(view.getCurrentPage() - 1)%>">上一页</a> </span>
						 <%} %>
						 <% 
						 if ((view.getCurrentPage() + 1) > view.getPageCount()) { 
						  %>
						<span class="dclass_pager_nav"> <a href="web/productmana/selectpublishpro.jsp?page=<%=view.getPageCount() %>">下一页</a>  </span>
						<%}else{ %>
						<span class="dclass_pager_nav"> <a href="web/productmana/selectpublishpro.jsp?page=<%=view.getCurrentPage()+1 %>">下一页</a>  </span>
						<%} %>
						<span class="dclass_pager_nav"> <a href="web/productmana/selectpublishpro.jsp?page=<%=view.getPageCount() %>">末页</a>  </span>
